...
export default function CareerPage() {
  ...
  const loadData = async () => {
    try {
      ...
      const careerBoard = boards.find((b: Board) => b.slug === 'career');

      if (careerBoard) {
        setBoard(careerBoard);
        // 获取该板块的帖子
        const response = await postsApi.getPosts({ boardId: careerBoard.id });
      ...
      <div className="p-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-vscode-text-secondary text-sm">
                {board?.description || '面试经验、求职经历、薪资分享'}
              </p>
            </div>
          </div>
          <Button>
        ...
        {isLoading ? (
          ...
          <div className="text-center py-12">
            <Briefcase className="w-16 h-16 mx-auto text-vscode-text-tertiary mb-4" />
            <p className="text-vscode-text-secondary mb-4">
              还没有就业分享，成为第一个分享的人吧！
            </p>
            <Button>发布第一个经验</Button>
          </div>
        )}
      </div>
    </VSCodeLayout>
